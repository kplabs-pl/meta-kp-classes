# From: https://github.com/phytec/meta-phytec
...
FITIMAGE_IMAGE_SUFFIX ??= ".fitimage"
...
def fitimage_emit_section_config(d,fd,dtb,teecount,kernelcount,ramdiskcount,setupcount,bootscriptid,i):
    ...
    conf_sign_keyname = get_key_name_hint(d.getVar("FITIMAGE_SIGN_KEY_PATH", True) or "")
...
do_deploy() {
    ...
    linux_bin_base_name="${PN}-${PV}-${PR}-${MACHINE}${IMAGE_VERSION_SUFFIX}${FITIMAGE_IMAGE_SUFFIX}"
