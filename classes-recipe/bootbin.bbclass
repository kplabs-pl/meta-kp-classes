...
QEMU_FLASH_IMAGE_NAME ?= "qemu-${BOOTBIN_NAME}-${QEMU_FLASH_TYPE}-${MACHINE}${IMAGE_VERSION_SUFFIX}"

BOOTBIN_BASE_NAME ?= "BOOT-${BOOTBIN_NAME}-${MACHINE}${IMAGE_VERSION_SUFFIX}"

OUTPUT_DIR ?= "${DEPLOYDIR}/bootbins"

do_deploy() {
    install -d ${OUTPUT_DIR}
    install -m 0644 ${B}/BOOT.bin ${OUTPUT_DIR}/${BOOTBIN_BASE_NAME}.bin
    ln -sf ${BOOTBIN_BASE_NAME}.bin ${OUTPUT_DIR}/BOOT-${BOOTBIN_NAME}-${MACHINE}.bin
    ln -sf ${BOOTBIN_BASE_NAME}.bin ${OUTPUT_DIR}/boot-${BOOTBIN_NAME}.bin
}

do_deploy:append:versal () {

    install -m 0644 ${B}/BOOT_bh.bin ${OUTPUT_DIR}/${BOOTBIN_BASE_NAME}_bh.bin
    ln -sf ${BOOTBIN_BASE_NAME}_bh.bin ${OUTPUT_DIR}/BOOT-${BOOTBIN_NAME}-${MACHINE}_bh.bin

    install -m 0644 ${B}/qemu-${QEMU_FLASH_TYPE}.bin ${OUTPUT_DIR}/${QEMU_FLASH_IMAGE_NAME}.bin
    ln -sf ${QEMU_FLASH_IMAGE_NAME}.bin ${OUTPUT_DIR}/qemu-${BOOTBIN_NAME}-${QEMU_FLASH_TYPE}-${MACHINE}.bin
}
